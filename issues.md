...
| ISS-006 | フィルムエフェクトレイヤーがグループに格納されない | フィルムエフェクト機能実行時に「フィルムエフェクト」グループは作成されるが、実際に生成されるレイヤー（ハレーション、グレイン、ドリーミーヘイズなど）がそのグループ内に配置されていない | レイヤー作成時にグループへの移動処理を追加。各エフェクトレイヤー生成後に適切なグループに移動させる処理を実装する | nob | 🔴 未対応 | 🔴 High | 2025-06-08 | 2025-06-08 |
